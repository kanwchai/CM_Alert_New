package com.saikaew_rus.cm_alert;

/**
 * Created by NB_A on 18/10/2559.
 */
public class TB_2_DUE_OF_PART_FIX {
    // Labels table name
    public static final String TABLE = "DUE_OF_PART_FIX";

    // Labels Table Columns names
    public static final String Fix_Due_Id = "Fix_Due_Id";
    public static final String Part_Id = "Part_Id";
    public static final String Car_Id = "Car_Id";
    public static final String Fix_Due_Kilo = "Fix_Due_Kilo";
    public static final String Fix_Due_Date = "Fix_Due_Date";
    public static final String Fix_Due_Status = "Fix_Due_Status";
    public static final String Fix_Due_Show = "Fix_Due_Show";

    public static final String CountKilo = "CountKilo";
    public static final String CountDate = "CountDate";
    public static final String SetColor = "SetColor";

    //property help us to keep data
    public int fix_Due_Id;
    public int part_Id;
    public int car_Id;
    public int fix_Due_Kilo;
    public int fix_Due_Date;
    public String fix_Due_Status;
}
